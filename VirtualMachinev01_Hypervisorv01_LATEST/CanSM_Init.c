...
#define CANSM_START_SEC_VAR_CLEARED_UNSPECIFIED
#include "CanSM_MemMap.hpp"
P2CONST(Type_CfgSwcServiceSmCan_st, CANSM_VAR, CANSM_APPL_CONST) CfgSwcServiceSmCan_castConfigSet_pcst;
#define CANSM_STOP_SEC_VAR_CLEARED_UNSPECIFIED
#include "CanSM_MemMap.hpp"
...
#include "CanSM_MemMap.hpp"
FUNC(void, CANSM_CODE) CanSM_Init(const Type_CfgSwcServiceSmCan_st* ConfigPtr){
   ...
   CfgSwcServiceSmCan_castConfigSet_pcst = &CfgSwcServiceSmCan_castConfigSet[0];
    (void)ConfigPtr;
#else
   CfgSwcServiceSmCan_castConfigSet_pcst = ConfigPtr;
#endif

   CanSM_Network_pcst = CfgSwcServiceSmCan_castConfigSet_pcst->CanSM_NetworkConf_pcst;

#if(CANSM_DSM_REINIT_ENABLED != STD_OFF)
   CanSM_NetworkDem_pst = CfgSwcServiceSmCan_castConfigSet_pcst->CanSMNetworkConfDEM_pst;
#endif

   CanSM_ModeRepeatReq_Max_u8   = CfgSwcServiceSmCan_castConfigSet_pcst->CanSMModeRequestRepetitionMax_u8;
   CanSM_ModeRepeatReq_Time_u16 = CfgSwcServiceSmCan_castConfigSet_pcst->CanSMModeRequestRepetitionTime_u16;
   for(CanSM_NetworkIdx_u8 =0; CanSM_NetworkIdx_u8 < CfgSwcServiceSmCan_castConfigSet_pcst->CanSM_SizeOfCanSMNetworks_u8 ; CanSM_NetworkIdx_u8++){
        ...
        switch(CanSM_PreNoCom_Substates){
            ...
            case CANSM_S_TRCV_NORMAL_WAIT:{
                if(CanSM_Trcv_ModeInd_ab[network]==TRUE){
                    if(CfgSwcServiceSmCan_castConfigSet_pcst->CanSM_NetworkConf_pcst[network].Trcv_hndle_u8 != 255){
