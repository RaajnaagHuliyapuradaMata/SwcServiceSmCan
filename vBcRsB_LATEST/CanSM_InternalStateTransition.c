...
FUNC(NetworkHandleType, CANSM_CODE) CanSM_GetHandle(VAR(NetworkHandleType, AUTOMATIC) ComMChannelId){
   ...
   for(loop_uo =0; loop_uo < CfgSwcServiceSmCan_castConfigSet_pcst->CanSM_SizeOfCanSMNetworks_u8 ; loop_uo++){
   	if(CfgSwcServiceSmCan_castConfigSet_pcst->CanSM_NetworkConf_pcst[loop_uo].ComM_channelId_uo == ComMChannelId){
   	    CanSM_NetworkIdx_u8 = (NetworkHandleType)loop_uo;
            break;
        }
        else{
            CanSM_NetworkIdx_u8 = CfgSwcServiceSmCan_castConfigSet_pcst->CanSM_SizeOfCanSMNetworks_u8 + (uint8)1;
        }
   }
   				...
   				if(CanSM_Ctrl_ModeInd_ab[network] == TRUE){
   					...
   					for(CanSM_Ctrl_index_u8 = 0; CanSM_Ctrl_index_u8 < CanSM_Network_pcst[network].SizeofController_u8 ;CanSM_Ctrl_index_u8++){
   						...
   						if(CanSM_Passive_b == TRUE){
   							(void)(CanIf_SetPduMode(CanSM_ControllerId_u8,EcuabCanIf_eModePdu_TX_OFFLINE_ACTIVE));
   						}
   						else{
